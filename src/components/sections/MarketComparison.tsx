...
import { marketKPIs, marketTrendData, marketChartKPIs, marketPriceComparison } from "../../data/mock-data"
        ...
        {marketKPIs.map((kpi) => (
          ...
          >
            ...
            <div className="flex items-end gap-1 h-8 mt-3">
              {[40, 60, 35, 80, 50, 70].map((h, i) => (
                <div
                  key={i}
                  className="flex-1 rounded-t-sm"
                  style={{ height: `${h}%`, backgroundColor: kpi.color, opacity: 0.6 }}
                />
              ))}
            </div>
          </div>
